...
st.title("🌱 Carbon Footprint Calculator (Advanced)")
...
fig = px.pie(
    ...
    title="Emission Breakdown by Activity",
    hole=0.4
)
st.plotly_chart(fig)
...
<h2>Emission Breakdown Chart</h2>
...
if st.button("Generate and Download PDF with Chart"):
    pdfkit.from_string(html_string, "carbon_footprint_report_with_chart.pdf")
    with open("carbon_footprint_report_with_chart.pdf", "rb") as f:
        st.download_button("Click to download PDF", f, file_name="carbon_footprint_report_with_chart.pdf")
